...
WORKDIR /usr/src/minetest
# Enable metrics listener on a nice address for Docker environments
RUN sed -i 's/127.0.0.1:8080/0.0.0.0:30000/' src/util/metricsbackend.cpp src/defaultsettings.cpp && \
	mkdir build && \
	cd build && \
	cmake .. \
		-DCMAKE_INSTALL_PREFIX=/usr/local \
