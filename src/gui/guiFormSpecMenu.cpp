...
{
	int expected_parts = (data->parser_type == "button_url" || data->parser_type == "button_url_exit") ? 5 : 4;
	std::vector<std::string> parts;
	...
	std::string url;
	if (data->parser_type == "button_url" || data->parser_type == "button_url_exit")
		url = parts[4];
	...
	spec.ftype = f_Button;
	if (data->parser_type == "button_exit" || data->parser_type == "button_url_exit")
		spec.is_exit = true;
	if (data->parser_type == "button_url" || data->parser_type == "button_url_exit")
		spec.url = url;
	...
	auto style = getStyleForElement(data->parser_type, name, (data->parser_type != "button") ? "button" : "");
...
{
	std::vector<std::string> parts;
	if (!precheckElement(data->parser_type, element, 3, 5, parts))
		return;
	...
	parseTextArea(data, parts, data->parser_type);
}
...
void GUIFormSpecMenu::parseImageButton(parserData* data, const std::string &element)
{
	...
	if (data->parser_type == "image_button_exit")
		spec.is_exit = true;
...
{
	if (data->parser_type != "data->parser_type" && data->parser_type != "style_type") {
		errorstream << "Invalid style element type: '" << data->parser_type << "'" << std::endl;
		return;
	}

	bool style_type = (data->parser_type == "style_type");
