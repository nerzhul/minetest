...
bool WorldSettings::load(bool return_on_config_read)
{
	...
	if (!exists(PLAYER_BACKEND_KEY)) {
        ...
        infostream << "No " << PLAYER_BACKEND_KEY << " key found, assuming files." << std::endl;

		if (!updateConfigFile()) {
			errorstream << "WorldSettings::load(): " << "Failed to update world.mt!" << std::endl;
		}
	}
    ...
    if (!exists(AUTH_BACKEND_KEY)) {
        ...
        infostream << "No " << AUTH_BACKEND_KEY << " key found, assuming files." << std::endl;
