...
PrometheusMetricCounter::PrometheusMetricCounter(const std::string &name,
...
void PrometheusMetricCounter::increment(double number)
{
	m_counter.Increment(number);
}

double PrometheusMetricCounter::get() const
{
	return m_counter.Value();
}

PrometheusMetricGauge::PrometheusMetricGauge(const std::string &name,
		const std::string &help_str,
		std::shared_ptr<prometheus::Registry> registry) :
		MetricGauge(),
...
void PrometheusMetricGauge::increment(double number)
{
	m_gauge.Increment(number);
}
void PrometheusMetricGauge::decrement(double number)
{
	m_gauge.Decrement(number);
}
void PrometheusMetricGauge::set(double number)
{
	m_gauge.Set(number);
}
double PrometheusMetricGauge::get() const
{
	return m_gauge.Value();
}

PrometheusMetricsBackend::PrometheusMetricsBackend() : MetricsBackend()
{
	std::string addr;
	if (!g_settings->getNoEx("prometheus_listener_address", addr))
		addr = "127.0.0.1:8080";
	m_exposer = std::unique_ptr<prometheus::Exposer>(new prometheus::Exposer(addr));
	m_registry = std::make_shared<prometheus::Registry>();
	actionstream << "Starting prometheus listener on " << addr << std::endl;
